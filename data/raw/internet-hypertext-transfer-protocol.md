...
source: "https://simple.wikipedia.org/wiki/Hypertext_Transfer_Protocol"
...
HTTPS is used by default by most web browsers (even if not requested by the user), assuming the web server supports it. If a web browser shows a website's URL starting with http: without an s before the colon (or an unlocked icon is seen), then that website is not secure, so passwords or other data sent to them can be stolen. HTTPS encrypts (hides) all the information that is both sent and received. This can, therefore, stop malicious users such as hackers from stealing the information. HTTPS uses port 443 for communication instead of port 80.
...
The request message (for HTTP/1.1 and older) contains the following (i.e. it's in plain text, meaning it is not encrypted; no longer used with HTTP/2 and later):
