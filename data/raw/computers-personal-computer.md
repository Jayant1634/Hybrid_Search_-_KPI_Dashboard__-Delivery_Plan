...
source: "https://simple.wikipedia.org/wiki/Personal_computer"
...
Operating Systems
Today, most PCs usually have a basic set of software called an operating system. The operating system is responsible for several tasks including providing a user interface (UI). The most popular operating system on PCs is Windows, sold by Microsoft Corporation. The first PCs usually had their operating system on a floppy disk. On x86 PCs, this was often MS-DOS. PCs made by a company called Apple Inc. use a different system of software called Mac OS that is also made by Apple Inc.
Many free operating systems are available. Some are called Linux operating systems. There are over 300 different Linux "distributions". Each one has a different purpose/feel. Ubuntu Linux is the most-used Linux distribution. Various other operating systems are available, many of which are based on Unix.


Parts of a personal computer
A modern PC has a minimum set of parts to be useful. The "base unit" or "tower" (case) houses the computer parts. Usually, a mouse and keyboard are used for input, however some operating systems need only a keyboard. A monitor or small TV is usually needed to view output. In a laptop computer these parts are all together.
...
Computers may be used for work, including doing research using the internet, keeping records; or writing documents. Other uses include communicating, with people across the world using Instant messaging, e-mail or Skype or recreation activity such as playing computer games. Some PCs are designed for a specific purpose, for example gaming PCs have powerful graphics cards for 3D games, whereas workstations have reliable hardware for important work that a business might depend on.
